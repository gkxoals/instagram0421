...
public class DmService {
    ...
    public List<UserListDTO> getUserListForCurrentUser() {
        ...
        return rooms.stream()
                ...
                // ✅ 메시지 존재하는 방만
                .filter(room -> chatMessageRepository.existsByChatRoom_Id(room.getId())).map(room -> {
                    ...
                    return new UserListDTO(partner.getUserId(), nickname, profileImage, room.getId(), lastMessage);
    ...
    public String getPartnerNickname(Long roomId, Long myId) {
        ...
        if (!room.getUser1().getUserId().equals(myId) && !room.getUser2().getUserId().equals(myId)) {
            return "알 수 없음"; // 접근 불가
        }

        User partner = room.getOtherParticipant(myId);
        return profileRepository.findByUserId(partner).map(Profile::getNickname).orElse("알 수 없음");
    }
    ...
    public void deleteChatRoom(Long roomId, Long currentUserId) {
        ...
        chatRoomRepository.hardDeleteByChatRoomId(chatRoom.getId());
